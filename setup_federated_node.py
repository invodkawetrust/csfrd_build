...
def do_backend_rpc_setup(run_as_user, branch, base_path, dist_path, run_mode, backend_rpc_mode):
    ...
    if backend_rpc_mode == 's': #saffroncoind
        ...
        runcmd("wget -O /tmp/saffroncoind.tar.gz https://github.com/saffroncoin/saffroncoin/releases/download/v%s/saffroncoin-binary-%s.tar.gz --no-check-certificate" % (SAFFRONCOIND_VER, SAFFRONCOIND_VER))
...
def do_armory_utxsvr_setup(run_as_user, base_path, dist_path, run_mode, enable=True):
    ...
    runcmd("wget -O /tmp/armory.deb https://s3.amazonaws.com/bitcoinarmory-releases/armory_0.92.1_ubuntu-64bit.deb --no-check-certificate")
