...
def get_paths(with_counterblockd):
    ...
    if with_counterblockd:
        ...
        paths['python_path.csfrblockd'] = os.path.join(paths['env_path.csfrblockd'], "Scripts" if os.name == "nt" else "bin", "python.exe" if os.name == "nt" else "python")
        ...
        if ubuntu_release in ("14.04", "13.10"):
            ...
            if with_counterblockd:
                ...
                if assume_yes:
                    ...
                    while True:
                        db_locally = input("csfrblockd: Run mongo and redis locally? (y/n): ")
...
def setup_startup(paths, run_as_user, with_counterblockd, with_testnet, assume_yes):
    if os.name == "posix":
        ...
        if with_counterblockd:
            ...
            f.close()
            runcmd("chmod +x /usr/local/bin/csfrblockd")
    elif os.name == "nt":
        #create a batch script
        batch_contents = "echo off%sREM Launch csfrd (source build) under windows%s%s %s %%*" % (
            os.linesep, os.linesep, os.path.join(paths['sys_python_path'], "python.exe"), os.path.join(paths['base_path'], "run.py"))
        f = open("%s" % os.path.join(os.environ['WINDIR'], "csfrd.bat"), "w")
    ...
    if assume_yes:
        ...
        while True:
            start_choice = input("Start csfrd automatically on system startup? (y/n): ")
    ...
    if os.name == "nt":
        ...
        scut = ws.CreateShortcut(os.path.join(buf.value, 'run_csfrd.lnk'))
        scut.TargetPath = '"c:/Python32/python.exe"'
        scut.Arguments = os.path.join(paths['base_path'], 'run.py') + ' server'
        scut.Save()
        if with_testnet:
            ws = win32com.client.Dispatch("wscript.shell")
            scut = ws.CreateShortcut(os.path.join(buf.value, 'run_csfrd_testnet.lnk' % s))
            scut.TargetPath = '"c:/Python32/python.exe"'
            scut.Arguments = os.path.join(paths['base_path'], 'run.py') \
                + (' --testnet --data-dir="%s" server' % data_dir, _get_app_cfg_paths('csfrd-testnet', run_as_user))
            scut.Save()
    else:
